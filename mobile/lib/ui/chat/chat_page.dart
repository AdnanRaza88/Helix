...
class ChatPage extends StatefulWidget {
  const ChatPage({required this.llm});
  final HelixLlm llm;
  @override
...
class ChatPageState extends State<ChatPage> {
  ...
  void initState() {
    super.initState();
    widget.llm.bindConfirm(_confirmMutation);
    _loadSessions();
  }
  ...
  Future<void> _send({String? override, String? parentId}) async {
    ...
    try {
      ...
      final reply = await widget.llm.runWithTools(
        text,
        history: prior,
        abort: abort,
        onDelta: (d) {
          ...
          child: Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: widget.llm.isLive ? H.success.withValues(alpha: 0.2) : H.purple.withValues(alpha: 0.25),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: widget.llm.isLive ? H.success : H.glassBorder),
              ),
              child: Text(widget.llm.liveLabel, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: widget.llm.isLive ? H.success : H.purpleSoft)),
            ),
          ]),
        ),
        if (active != null)
  ...
  Widget _bubble(ChatMessage m) {
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isUser ? H.purple.withValues(alpha: 0.35) : H.glass,
          ...
          children: [
            if (m.tools.isNotEmpty)
              Padding(
                ...
                child: Wrap(spacing: 4, children: m.tools.map((t) => Chip(label: Text(t, style: const TextStyle(fontSize: 10)), visualDensity: VisualDensity.compact)).toList()),
              ),
            if (isUser)
              SelectableText(m.text, style: const TextStyle(color: H.text, height: 1.4))
            else
              MarkdownBody(data: m.text.isEmpty ? '…' : m.text, styleSheet: MarkdownStyleSheet(p: const TextStyle(color: H.text, height: 1.45), code: const TextStyle(backgroundColor: Color(0x33000000), color: H.purpleSoft))),
            const SizedBox(height: 6),
            Row(children: [
              if (m.status == 'streaming') const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2)),
              const Spacer(),
              IconButton(icon: const Icon(Icons.copy, size: 16, color: H.textMuted), onPressed: () => _copy(m), visualDensity: VisualDensity.compact),
              if (isUser) IconButton(icon: const Icon(Icons.edit_outlined, size: 16, color: H.textMuted), onPressed: () => _edit(m), visualDensity: VisualDensity.compact),
              if (isUser) IconButton(icon: const Icon(Icons.refresh, size: 16, color: H.textMuted), onPressed: () => _resend(m), visualDensity: VisualDensity.compact),
              if (!isUser && m.status == 'error') IconButton(icon: const Icon(Icons.replay, size: 16, color: H.pink), onPressed: () => _retry(m), visualDensity: VisualDensity.compact),
              IconButton(icon: const Icon(Icons.delete_outline, size: 16, color: H.textMuted), onPressed: () => _deleteMessage(m), visualDensity: VisualDensity.compact),
            ]),
          ],
        ),
      ),
